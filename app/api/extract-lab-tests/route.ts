...
[
  ...
  },
  {
    ...
    const completion = await callLLM({
      useCase: 'EXTRACT_LAB_TESTS',
      messages: [
        { role: "system", content: "You are a medical assistant specialized in extracting structured laboratory test data from dermatology diagnoses. Always return valid JSON arrays only." },
        { role: "user", content: extractionPrompt }
      ],
      maxTokens: 1500,
      timeoutMs: 120_000,
    })
    console.log(`[llm] use=EXTRACT_LAB_TESTS provider=${completion.provider} model=${completion.model} latency=${completion.latencyMs}ms`)
