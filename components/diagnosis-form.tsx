...
interface DiagnosisFormProps {
 ...
 onNext: () => void
 /**
  * The doctor never reads this screen: it runs, then hands over to the medical
  * record. The staged reveal is skipped and onNext is called as soon as the
  * data has reached the parent — never before, or the record would mount with
...
export default function DiagnosisForm({
 ...
 /** Guards the automatic hand-over: the notify effect fires more than once. */
 const handedOverRef = useRef(false)
 const [apiTestResult, setApiTestResult] = useState<string | null>(null)
 const [hasAutoGenerated, setHasAutoGenerated] = useState(false)

 // Phase 2.D.F — refs persist across useEffect re-runs so the handoff
 // setTimeout survives a React dependency change (clinicalData / questionsData
 // arriving as the parent state fans out). The timer is single-shot per
 // mount via handoffScheduledRef; cleanup happens once at unmount in the
 ...
 // Structured triage block from the diagnostic API. It drives the triage
 // banner in the report; without it here the block was dropped by the
 // whitelists below and the banner could never render.
 const [triageAssessment, setTriageAssessment] = useState<any>(null)
 ...
 // Restore triage so resuming a consultation keeps its banner. Older
 // saves nested the raw payload under completeData — read that too.
 const savedTriage =
   savedData.diagnosisData.triage_assessment ||
   savedData.diagnosisData.completeData?.triage_assessment ||
   null
 if (savedTriage) setTriageAssessment(savedTriage)
 ...
 // Structured triage — required by the report's triage banner and
 // persisted for the report viewer / audit.
 ...
 // Handed over from HERE, and nowhere earlier: this is the moment the parent
 // has the diagnosis. Advancing on the API response instead would unmount
 // this component before the effect ran, and the medical record would mount
 // with no prescriptions, no triage and no references.
 if (autoAdvance && !handedOverRef.current) {
 handedOverRef.current = true
 console.log('🩺 Diagnosis ready — handing over to the medical record')
 onNext()
 }
 }
 }, [diagnosis, diagnosticReasoning, expertAnalysis, mauritianDocuments, documentsGenerated, currentMedicationsValidated, medications, combinedPrescription, evidenceReferences, ragUsed, ragMetadata, triageAssessment, autoAdvance])
 ...
 // Phase 2.D.F — unmount-only cleanup for the handoff timer. Runs on
 // unmount, NOT on auto-gen useEffect re-runs, so a dependency change
 // (e.g. clinicalData arriving while the timer is pending) does not kill
 // the in-flight schedule. This is the bug 2.D.E shipped by attaching
 // the cleanup directly to the auto-gen useEffect.
 useEffect(() => {
 return () => {
 ...
 useEffect(() => {
 const currentConsultationId = consultationId || consultationDataService.getCurrentConsultationId()
 const handoffFlag = typeof window !== 'undefined'
   ? sessionStorage.getItem('tibokHandoffJustHydrated')
   : null
 const isHandoffMount = !!handoffFlag && handoffFlag === currentConsultationId
 ...
 chiefComplaint: clinicalData?.chiefComplaint || '(empty)',
 isHandoffMount,
 handoffFlag,
 ...
 // ===== Normal flow trigger — UNCHANGED =====
 // Doctor filled steps 0/1/2 himself; props arrive synchronously through
 // the parent state setters in app/page.tsx; chiefComplaint guard is
 // legitimate to avoid an expensive LLM call on a half-typed form.
 if (!hasAutoGenerated && !diagnosis && patientData && clinicalData &&
 ...
 // ===== Phase 2.D.E/F — Handoff path: defensive force-trigger =====
 // Reasoning (2.D.D failed because):
 //  - patientData / clinicalData / questionsData arrive here as PROPS from
 //    app/page.tsx parent state, NOT as local state with setters.
 //  - When this useEffect first fires, the parent's setState may not yet
 //    have fanned the freshly hydrated data into our prop closure, so the
 //    gen function early-returned at L933 with "Missing required data".
 //
 // Fix: pull data straight from consultationDataService (synchronous,
 // already populated by app/page.tsx during the handoff hydration) and
 // pass it as overrides to the gen function. The 1.5s setTimeout is a
 // small cushion in case the service write hasn't flushed yet.
 //
 // Phase 2.D.F: timer + scheduled flag live in refs that persist across
 // useEffect re-runs. The dependency-change re-run that 2.D.E ate (when
 // clinicalData arrived) used to clearTimeout the pending timer; with
 // refs, the timer survives, and the scheduled flag stops the branch
 // from re-entering on subsequent re-runs while still pending. Cleanup
 // is owned by the dedicated unmount useEffect above, NOT by this one.
 if (isHandoffMount && !diagnosis && !hasAutoGenerated && !handoffScheduledRef.current) {
 console.log('🩺 [Diagnosis] Handoff mount — preparing force-trigger')
 handoffScheduledRef.current = true
 sessionStorage.removeItem('tibokHandoffJustHydrated') // single-shot

 // Single-shot React state guard so the normal-flow branch can't
 // re-arm before our timer fires.
 setHasAutoGenerated(true)
 // Reset stuck states defensively (if a previous attempt left them on).
 setLoading(false)
 setError(null)
 ...
 handoffTimerRef.current = setTimeout(() => {
   ...
   try {
     ...
     const overrides = {
       patientData: serviceData.patientData ?? patientData,
       clinicalData: serviceData.clinicalData ?? clinicalData,
       questionsData: serviceData.questionsData ?? questionsData,
     }
     console.log('🩺 [Diagnosis] Calling generateCompleteDiagnosisAndDocuments() with service-fresh overrides:', {
       hasPatient: !!overrides.patientData,
       hasClinical: !!overrides.clinicalData,
       hasQuestions: !!overrides.questionsData,
       chiefComplaint: overrides.clinicalData?.chiefComplaint || '(empty)',
     })
     generateCompleteDiagnosisAndDocuments(overrides)
   } catch (err) {
     console.error('❌ [Diagnosis] Handoff force-trigger threw:', err)
     // Re-arm the manual button + allow a fresh user-driven retry.
     setHasAutoGenerated(false)
     handoffScheduledRef.current = false
   }
 }, 1500)
 }
 // No cleanup return here — see the unmount-only useEffect above.
 }, [hasAutoGenerated, diagnosis, patientData, clinicalData, questionsData, consultationId])
 ...
 // Thirteen seconds of progressive reveal is worth its cost when a doctor
 // reads the result; it is thirteen seconds of nothing when they are being
 // taken straight to the record.
 const stage = (fn: () => void, ms: number) => {
   if (autoAdvance) { fn(); return }
   setTimeout(fn, ms)
 }
 ...
 // Triage block — captured immediately (not on a timer) so the report
 // always has it, even if the doctor advances before the reveal finishes.
 if (data.triage_assessment) setTriageAssessment(data.triage_assessment)
 ...
 rag_used: data.rag_used,
 rag_metadata: data.rag_metadata,
 // Structured triage — feeds the triage banner in the report.
 ...
 const generateCompleteDiagnosisAndDocuments = async (overrides?: {
   patientData?: any
   clinicalData?: any
   questionsData?: any
 }) => {
 // Phase 2.D.E — Optional `overrides` allow the handoff path to bypass the
 // React prop closure (which can be stale when this is called from inside a
 // useEffect that fired before the parent's setState fanned out) and inject
 // service-fresh data directly. Normal-flow callers omit overrides and the
 // function reads from props as before.
 const effectivePatientData = overrides?.patientData ?? patientData
 const effectiveClinicalData = overrides?.clinicalData ?? clinicalData
 const effectiveQuestionsData = overrides?.questionsData ?? questionsData

 console.log('🩺 ========== STARTING DIAGNOSIS GENERATION ==========')
 console.log('📋 Patient Data:', {
 hasData: !!effectivePatientData,
 age: effectivePatientData?.age,
 sex: effectivePatientData?.sex,
 name: `${effectivePatientData?.firstName} ${effectivePatientData?.lastName}`,
 fromOverride: !!overrides?.patientData,
 })
 console.log(' Clinical Data:', {
 hasData: !!effectiveClinicalData,
 chiefComplaint: effectiveClinicalData?.chiefComplaint,
 symptoms: effectiveClinicalData?.symptoms?.length || 0,
 fromOverride: !!overrides?.clinicalData,
 })
 console.log('❓ Questions Data:', {
 responses: effectiveQuestionsData?.responses?.length || 0,
 fromOverride: !!overrides?.questionsData,
 })

 if (!effectivePatientData || !effectiveClinicalData) {
 ...
 console.log('   📋 patientData.currentMedications:', effectivePatientData?.currentMedications)
 console.log('   📋 patientData.current_medications:', effectivePatientData?.current_medications)
 console.log('   📝 patientData.currentMedicationsText:', effectivePatientData?.currentMedicationsText)
 console.log('   ✅ Is Array?:', Array.isArray(effectivePatientData?.currentMedications))
 console.log('   ✅ Length:', effectivePatientData?.currentMedications?.length || 0)
 ...
 clinicalData: effectiveClinicalData,
 ...
 {/* Named after where the doctor lands, not after the step they no longer
     read: this screen exists only to hold their attention while the record
     is built. */}
 {autoAdvance ? "Préparation du dossier médical…" : "IA by Tibok in Action"}
 </CardTitle>
 </CardHeader>
