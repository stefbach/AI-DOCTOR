...
export default function DiagnosisForm({
 ...
 useEffect(() => {
 // Phase 2.D.D — handoff bypass.
 // When the doctor lands on this form via the nurse-handoff path, the
 // hub has already hydrated patient_data / clinical_data / questions_data
 // from TIBOK and set sessionStorage.tibokHandoffJustHydrated to the
 // current consultationId in app/page.tsx. We trust that hydration as
 // proof that step 0/1/2 are complete and bypass the chiefComplaint
 // guard which otherwise blocks generation when the nurse's saved
 // payload doesn't surface chiefComplaint at the expected top-level path.
 //
 // Idempotence: the flag is removed the moment we trigger generation,
 // so a subsequent re-render or a page refresh on this same step
 // proceeds via the normal-flow guards (which check hasAutoGenerated +
 // chiefComplaint) and never re-fires.
 const currentConsultationId = consultationId || consultationDataService.getCurrentConsultationId()
 ...
 patientData &&
 clinicalData &&
 (clinicalData.chiefComplaint || isHandoffMount)) {

 if (isHandoffMount) {
   console.log('🩺 [Diagnosis] Handoff mount detected, forcing auto-generation')
 ...
 }, [hasAutoGenerated, diagnosis, patientData, clinicalData, consultationId])
