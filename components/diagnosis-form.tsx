...
export default function DiagnosisForm({
 ...
 // ===== Phase 2.D.E — Handoff path: defensive force-trigger =====
 // Reasoning (Phase 2.D.D failed because):
 ...
 //  - When the handoff useEffect fires, the parent's setState may not yet
 //    have fanned the freshly hydrated data into our prop closure.
 //  - The previous attempt called generateCompleteDiagnosisAndDocuments()
 //    which then closure-read stale (null) props, hit the early-return at
 //    L933 with setError("Missing…"), and left hasAutoGenerated=true so
 //    no retry was possible.
 //
 ...
 // small extra cushion in case the service write hasn't flushed when
 // this effect first runs.
 if (isHandoffMount && !diagnosis && !hasAutoGenerated) {
 ...
 // Single-shot guard so this branch doesn't re-arm on the next render
 // while we wait for the timer.
 setHasAutoGenerated(true)
 // If a previous failed run left loading=true (early-return path
 // doesn't reset it; current code doesn't set it before the return,
 // but a future refactor might — defensive).
 setLoading(false)
 setError(null)
 ...
 const timer = setTimeout(() => {
   try {
     ...
     // Re-arm the manual button by resetting the single-shot guard.
